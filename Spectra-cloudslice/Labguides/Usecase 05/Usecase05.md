...
5. Wait for the Codespaces environment to setup .It takes few minutes to setup completely
...
1. After selecting subscription and location please **Create a New resource group**.
    ...
    - Select a Location for resource group : **Australia East** if it fails try with **Central US**

    - Enter the name of the Resource Group : Press **Enter**    

      ![](./media/new38.png)

6. This deployment will take *20-25 minutes* to provision the resources in your account and set up the solution with sample data.
...
9. Run the following command to install PowerShell in Codespaces to enable execution of PowerShell scripts within your development environment.
...
4. Run the following command to log in to your Azure account and authenticate your session.
...
10. Enter your email address for certificate management, then open the provided URL and enter the displayed code to complete Azure login authentication.
...
7. Click **Go to Foundry portal** to verify that the model has been successfully deployed.
...
5. Uploading the documents may take approximately 10–13 minutes to complete.
...
3. Enter  **Analyze the two annual reports and compare the positive and negative outcomes YoY. Show the results in a table.** and select **Send**.
...
7. Enter  **Can you summarize and compare the tables on page 10 and 11?** and select **Send**.

     ![](./media/image72.png)

8. Review the summarized comparison.

     ![](./media/image73.png)

9. Close the pop-up viewer
...
>**Note:** Before deleting the resources please make sure to validate the previous tasks.
...
documents such as housing reports and contracts, generate summaries, perform comparisons, and extract key information like buyer/seller details and financial insights. This demonstrates the practical value of AI in transforming unstructured data into actionable intelligence.
